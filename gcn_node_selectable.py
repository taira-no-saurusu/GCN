...
def get_random_train_mask(arraySize,num_train,TRUE_LABEL):
    ...
    # 例外処理
    if num_train[0] > 17 or num_train[1] > 17:
    ...
    for i in range(num_train[0]):

        index = random.choice(list[0])
        mask[index] = True
        list[0].remove(index)

    for i in range(num_train[1]):

        index = random.choice(list[1])
        mask[index] = True
        list[1].remove(index)
...
"""
def exec_to_kmedoids(times, TRAIN_ALL, DEFAULT, NUM_TRAIN, EPOCH, VIEW_TRAIN,VIEW_CLUSTERING, N_CLUSTER, TRUE_LABEL,METHOD,SHOW):
