...
def get_adj_index(list):
    l = random.choice(list)

    return random.choice(random.choice(list))
...
def get_random_train_mask(arraySize,num_train,TRUE_LABEL):
    ...
    list = [[i for i, x in enumerate(TRUE_LABEL) if x == 0], [i for i, x in enumerate(TRUE_LABEL) if x == 1], [
        i for i, x in enumerate(TRUE_LABEL) if x == 2]]
...
def exec_to_kmedoids(times, NUM_TRAIN, EPOCH, VIEW_TRAIN,VIEW_CLUSTERING, N_CLUSTER, TRUE_LABEL,METHOD,DATA=None):
    ...
    for i in range(times):
        ...
        #クラスタリング実行
        if METHOD=="kmedoids":
            pred = KMedoids(n_clusters=N_CLUSTER).fit_predict(embedded_vector_matrix)

        elif METHOD=="kmeans":
            pred = KMeans(n_clusters = N_CLUSTER).fit_predict(embedded_vector_matrix)
        else :
